...
exports.updateDoc = function (doc, user, time, cfg, tfile, readme, options, callback) {
    ...
    var publishedVersions = _(doc.versions).chain()
                            .filter(function (v) {
                                return (!v.state) || (v.state && v.state !== 'pullrequest')
                            })
                            .map(function (v) {
                                return v.version;
                            }).value();

    var highest = versions.max(publishedVersions);

    if (highest === cfg.version) {
        exports.updateTopLevelDoc(doc, cfg.version, cfg);
    }
...
/**
 * Updates a document to promote a pull request based on the
 * parameters entered.
 *
 * @param {String} repository - the repository.
 * @param {Object} options - the options for the pull request to accept.
 * @param {Function} callback
 */

exports.acceptPull = function (repository, options, callback) {
    if (!callback) {
        callback = options;
        options = {};
    }
    ...
    async.series({
        get: async.apply(exports.get, repourl, options.package.name),
        session: function (cb) {
    ...
    function (err, results) {
        ...
        if (!doc) {
            return callback(new Error('Could not find ' + options.package.name + ' to accept the pull request for version ' + options.package.version));
        }
        if (!doc.versions[options.package.version]) {
            return callback(new Error('Could not find submitted pull request for version ' + options.package.version));
        }

        var user = results.session[0].userCtx.name;
        var db = couchdb(repourl);
        var pullVersion = options.package.version;

        doc.time.modified = utils.ISODateString(time);
        doc.time[pullVersion] = utils.ISODateString(time);

        delete doc.versions[pullVersion].state;
        doc.activity.push({
            user: user,
            time: doc.time.modified,
            action: 'pullaccepted',
        ...
        var publishedVersions = _(doc.versions).chain().filter(function (v) {
                                    return (!v.state) || (v.state && v.state !== 'pullrequest')
                                }).map(function (v) {
                                    return v.version;
                                }).value();
        ...
        if (highest === pullVersion) {
            exports.updateTopLevelDoc(doc, pullVersion, doc.versions[pullVersion])
        }

        db.save(options.package.name, doc, callback);
    });
}

exports.updateTopLevelDoc = function (doc, version, latestVersion) {
    doc.tags = doc.tags || {};
    doc.tags.latest = version;

    doc.name = latestVersion.name;
    doc.description = latestVersion.description;
    doc.keywords = latestVersion.keywords;
    doc.categories = latestVersion.categories;
    doc.homepage = latestVersion.homepage;
    doc.github = latestVersion.github;
}
...
exports.createDoc = function (user, time, cfg, tfile, readme, options, callback) {
    var doc = {
        _id: cfg.name,
        name: cfg.name,
        type: 'package',
    ...
    exports.updateDoc(doc, user, time, cfg, tfile, readme, options, callback);
};
...
exports.publish = function (path, repository, /*opt*/options, callback) {
    ...
    settings.load(path, function (err, cfg) {
        ...
        function (err, results) {
            ...
            if (!curr && options.pullrequest) {
                return callback(new Error('You can\'t submit a pull request for ' + cfg.name +  ' as the package has not been published.'));
            }
            ...
            if (!curr) {
                return exports.createDoc(
                    user, time, cfg, tfile, readme, options, function (err, doc) {
            ...
            return exports.updateDoc(
                curr, user, time, cfg, tfile, readme, options, function (err, doc) {
                    db.save(cfg.name, doc, callback);
                }
            );
        });
        ...
        exports.get(repo, name, true, function (err, doc) {
            ...
            if (doc && doc.versions) {
                for (var k in doc.versions) {
                    if (!versions[k] && !doc.versions[k].state !== 'pullrequest') {
                        versions[k] = {
                            repository: repo,
