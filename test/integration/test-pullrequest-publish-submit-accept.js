...
exports['publish, pull request submitted then accept valid pull request'] = function (test) {
    test.expect(5);
    ...
    async.series([
        ...
        async.apply(utils.runJam, ['pullrequest', 'package-four@2.0.0', '--accept'], {env: ENV}),
        function (cb) {
            couchdb(TESTDB).get('package-four', function (err, doc) {
                ...
                test.equal(doc.versions['2.0.0'].description, 'Test package four version two pull request.');
                test.equal(doc.versions['2.0.0'].state, null);
                test.equal(doc.tags.latest, '2.0.0');
